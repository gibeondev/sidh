      ...
      <div className="mx-3 mt-4 rounded-lg bg-slate-700/50 px-3 py-3">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="truncate text-sm font-medium text-white">{user?.email ?? 'Orang Tua'}</p>
            <p className="truncate text-xs text-slate-400">{user?.role === 'PARENT' ? 'Orang Tua' : user?.role ?? '–'}</p>
          </div>
        </div>
      </div>
