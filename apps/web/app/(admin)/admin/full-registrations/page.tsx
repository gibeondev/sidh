...
type StatusFilterValue = ApplicationStatus | '';
...
export default function AdminFullRegistrationsPage() {
...
function ApplicationRow({
  ...
  const rs = row.registrationSubmission;
  const pr = row.preRegistration;
  const studentName = rs?.studentFullName ?? pr?.studentName ?? '–';
  const program = rs?.programChoice ?? pr?.programChoice ?? '–';
  const handleRowClick = (e: React.MouseEvent<HTMLTableRowElement>) => {
    const target = e.target as HTMLElement;
    if (target.closest('button') || target.closest('input[type="checkbox"]')) return;
    router.push(`/admin/full-registrations/${row.id}`);
  };
  return (
    <TableRow
